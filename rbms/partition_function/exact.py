...
def compute_partition_function_rbm(params: RBM, all_config: Tensor, context: Tensor | None = None) -> float:
    """Compute the exact log partition function. Supporting conditional context.
    ...
    if n_dim_config == n_hidden:
        energy = params.compute_energy_hiddens(h=all_config, context=context)
    elif n_dim_config == n_visible:
        energy = params.compute_energy_visibles(v=all_config, context=context)
    else:
        raise ValueError(
    ...
    return torch.logsumexp(-energy, dim=0)


def compute_partition_function(params: EBM, all_config: Tensor, context: Tensor | None = None) -> float:
    if isinstance(params, RBM):
        return compute_partition_function_rbm(params=params, all_config=all_config, context=context)
